"""
Offline preprocessing: raw greeting videos → NumPy sequences for training.

Pipeline per video:
  OpenCV read → MediaPipe Holistic (every skip_frame-th frame) → landmarks_data()
  → pad/truncate to 30 frames → save keypoint_data/<gesture>/<name>.npy

Run once before train.py; avoids re-running MediaPipe on every training epoch.
"""
...
def save_data(action, video_file, export_path, import_path, max_frame_length=30, skip_frame=2):
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        ...
        while cap.isOpened():
            ...
            # Sample every SKIP_FRAME-th frame to reduce redundancy (~15 poses/sec at 30 FPS)
            if ret and (frame_count % SKIP_FRAME == 0):
                ...
                # End of video OR collected 30 frames — pad if video was shorter than 30
